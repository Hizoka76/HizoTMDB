...
# Version: 23-02-21.0
    ...
    # Refonte de setDefaultValues et de resetDefaultValues sans changement pour utilisateur
    # Simplification de QCoreApplication.translate en translate
    # Ajout des signaux :
        # currentDataChanged = Signal(list)
        # currentInfoChanged = Signal(object)
        # currentValueChanged = Signal()
        # ils sont émit en même tempts que currentTextChanged(str)
    # Creation de setItemTextFromData pour maj les texts
    # dataIsChecked
    # dataIsPartiallyChecked
    # indexFromData
    # removeItemFromData
    # dataHighlighted
    # statusTipHighlighted
...
class QCheckComboBox(QComboBox):
    ...
    currentInfoChanged = Signal(object)
    currentValueChanged = Signal()
    dataHighlighted = Signal(str)
    statusTipHighlighted = Signal(str)
    ...
    def __init__(self, Parent=None, *args, **kwargs):
        ...
        # Connexion de currentTextChanged sur la fonction onCurrentTextChanged pour émettre les signaux personnalisés
    ...
    def contextMenuUpdate(self):
        ...
        # Translate bidon qui saute...
        text = translate("QCheckComboBox", "Copy values choosen")
        ...
        CopyAction = QAction(self.MenuIcons["Copy"], translate("QCheckComboBox", "Copy values choosen"), self.contextMenu)
        ...
        self.UndoAction = self.undoStack.createUndoAction(self.contextMenu, translate("QCheckComboBox", "Undo Action"))
        self.RedoAction = self.undoStack.createRedoAction(self.contextMenu, translate("QCheckComboBox", "Redo Action"))

        if self.UndoEnabled:
            self.contextMenu.addSection(translate("QCheckComboBox", "UnRedo Actions"))
        ...
        if self.CheckAllEnabled:
            self.contextMenu.addSection(translate("QCheckComboBox", "Check Boxes"))

            # Création et ajout de l'action de cochage
            AllCheck = QAction(self.MenuIcons["AllCheck"], translate("QCheckComboBox", "Check All Items"), self.contextMenu)
            ...
            if self.TristateMode:
                AllSemiCheck = QAction(self.MenuIcons["AllPatriallyCheck"], translate("QCheckComboBox", "Partially Check All Items"), self.contextMenu)
            ...
            AllUncheck = QAction(self.MenuIcons["AllUncheck"], translate("QCheckComboBox", "Uncheck All Items"), self.contextMenu)
        ...
        if len(self.DefaultValues):
            self.contextMenu.addSection(translate("QCheckComboBox", "Default Values"))

            ResetDefaultValues = QAction(self.MenuIcons["DefaultValues"], translate("QCheckComboBox", "Restore the default values"), self.contextMenu)
    ...
    def eventFilter(self, Object, Event):
        ...
        ### Cas du viewport
            ...
            if Event.type() == QEvent.MouseMove:
                ...
                # Gestion des connexions de survol
                Index = self.view().indexAt(Event.pos())

                if Index.isValid():
                    self.statusTipHighlighted.emit(self.model().data(Index, Qt.StatusTipRole))
                    self.dataHighlighted.emit(self.model().data(Index, Qt.UserRole))

                else:
                    self.statusTipHighlighted.emit("")
                    self.dataHighlighted.emit("")
            ...
            # Lors de l'entrée au dessus des items
    ...
    def shortcutEvent(self, Event):
        ...
        if Event.modifiers() == Qt.ControlModifier and Event.key() in [Qt.Key_A, Qt.Key_C, Qt.Key_P, Qt.Key_R, Qt.Key_Z]:
            ...
            elif Event.key() == Qt.Key_R and len(self.DefaultValues):
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def addItem(self, Text, Data=None, State=None, Icon=None, ToolTip=None, StatusTip=None):
        ...
        if Icon is not None:
            Item.setIcon(IconChecker(Icon))

        if StatusTip is not None:
            Item.setData(StatusTip, Qt.StatusTipRole)
    ...
    def addItems(self, Items):
        ...
        Les items sont de type : {text, data, state, icon, tooltip}"""
        ...
        for Item in Items:
            ...
            self.addItem(ItemCase["text"], ItemCase.get("data"), ItemCase.get("state"), ItemCase.get("icon"), ItemCase.get("tooltip"), ItemCase.get("statutstip"))
    ...
    def setDefaultValues(self, Values):
        ...
        # Réinitialisation de la liste par défaut
        self.DefaultValues = []
        ...
        for Item in Values:
            ...
            if Item.get("state") not in [Qt.Checked, Qt.PartiallyChecked]:
                continue

            # Si la valeur est vide on la saute
            if not Item.get("value"):
                continue

            # Si tout est bon, on le conserve
            self.DefaultValues.append(Item)
    ...
    def resetDefaultValues(self):
        ...
        # S'il y a bien une liste de valeur par défaut
        if len(self.DefaultValues):
            ...
            Checked = []
            PartiallyChecked = []
            Indexes = {Qt.Checked: [], Qt.PartiallyChecked: [] }

            # Tourne sur tous les items
            for Index in range(self.model().rowCount()):
                Item = self.model().item(Index)
                ItemText = Item.text()
                ItemData = Item.data(Qt.UserRole)

                # Boucle sur chaque valeur par défaut
                for DefaultValue in self.DefaultValues:
                    Value = DefaultValue.get("value")

                    # Si ce n'est pas une recherche par texte mais par index
                    if isinstance(Value, int):
                        # Si l'index correspond, on le prend
                        if Value == Index:
                            Indexes[DefaultValue.get("state")].append(Index)

                        break

                    # Mode avec prise en compte de la casse lors de la recherche du texte
                    if not DefaultValue.get("case"):
                        ItemText = ItemText.lower()
                        ItemData = ItemData.lower()
                        Value = Value.lower()

                    # Recherche du texte dans le texte de l'item et dans son data
                    if Value in (ItemText, ItemData):
                        # Ajout de l'item dans la liste à cocher ou semi-cocher
                        Indexes[DefaultValue.get("state")].append(Index)

                        break

            # Coche les différentes cases
            for State, SubIndexes in Indexes.items():
                if SubIndexes:
                    self.setStateItem(State, SubIndexes)
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def removeItemFromData(self, Data):
        """Fonction permettant de supprimer un item en se basant sur sa data."""
        ...
        for i in range(self.model().rowCount()):
            ...
            CheckData = self.model().item(i).data(Qt.UserRole)
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def setItemInfoFromData(self, Data, Info):
        """
            Fonction permettant de mettre à jour les informations d'un item en se basant sur sa data.
            Info est un dictionnaire pouvant contenir les clés text / icon / tooltip / state avec leur valeur.
        """
        # Tourne sur toutes les cases à cocher
        for i in range(self.model().rowCount()):
            ...
            if Item.data(Qt.UserRole) == Data:
                for Key, Value in Info.items():
                    if Key.lower() == "text":
                        ...
                        Item.setIcon(IconChecker(Value)) # QIcon(QPixmap(Icon))

                    elif Key.lower() == "tooltip":
                        Item.setToolTip(Value)

                    elif Key.lower() == "statustip":
                        Item.setData(Value, Qt.StatusTipRole)

                    elif Key.lower() == "state":
                        if Value in [Qt.Unchecked, Qt.Checked]:
                            Item.setIcon(IconChecker(Value))

                        elif Value == Qt.PartiallyChecked and  self.TristateMode:
                            Item.setIcon(IconChecker(Value))
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def dataIsChecked(self, Data):
        """Fonction permettant de savoir si une case est cochée."""
        # Tourne sur toutes les cases à cocher
        for i in range(self.model().rowCount()):
            Item = self.model().item(i)
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def dataIsPartiallyChecked(self, Data):
        """Fonction permettant de savoir si une coche est partiellement cochée."""
        # Tourne sur toutes les cases à cocher
        for i in range(self.model().rowCount()):
            Item = self.model().item(i)
    ...
    def indexFromData(self, Data):
        """Fonction permettant d'avoir l'index d'une case à cocher en se basant sur sa data."""
        # Tourne sur toutes les cases à cocher
        for i in range(self.model().rowCount()):
            Item = self.model().item(i)
    ...
    def onCurrentTextChanged(self, Text):
        """Fonction envoyant les nouveaux signaux en même temps que le signal original currentTextChanged."""
        self.currentDataChanged.emit(self.currentData())
        self.currentInfoChanged.emit(self.currentInfo())
